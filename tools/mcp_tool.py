...
def _interpolate_env_vars(value):
    ...
    Supports shell-style defaulting: ``${VAR:-default}``, ``${VAR-default}``,
    ``${VAR:+alt}``, ``${VAR+alt}``. Defaults may reference other ``${VAR}``.
    """
    if isinstance(value, str):
        ...
        def _replace(m):
            ...
            for op in (":-", ":+", "-", "+"):
                idx = expr.find(op)
                if idx != -1:
                    name, default = expr[:idx], expr[idx + len(op):]
                    cur = os.environ.get(name)
                    has = cur is not None and (cur != "" or op in ("-", "+"))
                    if op in (":+", "+"):
                        return _interpolate_env_vars(default) if has else ""
                    return cur if has else _interpolate_env_vars(default)
